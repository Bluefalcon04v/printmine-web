      ...
      {/* <div className="flex flex-col gap-1.5">
        ...
        <div className="flex gap-2">
          {["Circle", "Square", "Logo Cutout"].map((shape, i) => (
            <button
              className={`px-2 border border-neutral-200 text-xs hover:font-semibold py-0.5 cursor-pointer active:bg-black/20 active:scale-95 hover:scale-110 transition-all rounded-sm flex place-content-center place-items-center ${
                shape === selectShape
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="flex gap-4">
          {["10", "20", "30", "40", "50", "60"].map((quantity, i) => (
            <button
              className={`px-2 border border-neutral-200 text-base hover:border-neutral-400 hover:bg-neutral-100 py-0.5 cursor-pointer active:bg-black/20 active:scale-95 transition-all rounded-sm flex place-content-center place-items-center ${
                quantity === selectQuantity
                  ? "bg-black/10 text-black font-semibold shadow-sm "
                  : ""
              }`}
              onClick={() => setSelectQuantity(quantity)}
              key={i}
            >
              {" "}
              {quantity} Pieces
